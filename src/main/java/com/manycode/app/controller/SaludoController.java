...
//    	UserRating userRating = webClientBuilder.build().get().uri("http://localhost:8080/user/MANY")
    	UserRating userRating = webClientBuilder.build().get().uri("//aplimicroservcios12.demomicroservcios12.svc.cluster.local/user/MANY")
...
//    	UserRating userRating = webClientBuilder.build().get().uri("http://localhost:8080/user2/MANY")
    	UserRating userRating = webClientBuilder.build().get().uri("//aplimicroservcios12.demomicroservcios12.svc.cluster.local/user2/MANY")
    			.retrieve().bodyToMono(UserRating.class).block();
    	return (userRating.getUserId());    	
    }
